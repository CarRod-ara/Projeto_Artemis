// =============================================================================
// PROJETO DE ESTUDOS: Gerenciador de Banco de Dados (db.js) – Luminar PWA
// =============================================================================
// Este módulo implementa a camada de persistência local usando IndexedDB.
// IndexedDB é um banco de dados NoSQL embutido no navegador, ideal para PWAs
// que precisam armazenar dados estruturados offline sem depender de servidor.
//
// Conceitos importantes que você verá neste arquivo:
//   - Padrão Promise para operações assíncronas com callbacks do IndexedDB
//   - Criação de schema via evento "upgradeneeded"
//   - Uso de índices para buscas rápidas
//   - Tranzações readwrite vs readonly
//   - Isolamento de dados por usuário (userId em diversos métodos)
//   - Exportação/importação de dados como JSON (backup)
//
// O IndexedDB foi escolhido por ser a única solução offline nativa capaz
// de armazenar quantidades significativas de dados estruturados (ao contrário
// de localStorage, limitado a strings e ~5MB).
// =============================================================================

class LuminarDB {
    constructor() {
        this.dbName = 'LuminarDB';
        this.dbVersion = 2;        // Versão do schema — incrementar força upgrade
        this.db = null;            // Referência para a conexão aberta
    }

    // ========== INICIALIZAÇÃO ==========
    // Abre a conexão com o IndexedDB e cria o schema se necessário.
    // Retorna uma Promise para permitir await no código que chama.
    async init() {
        return new Promise((resolve, reject) => {
            ...
            // Este evento é disparado quando o banco é criado ou a versão muda.
            // É aqui que definimos as "tabelas" (object stores) e índices.
            request.onupgradeneeded = (event) => {
                const db = event.target.result;
                
                // Store: usuários do sistema
                if (!db.objectStoreNames.contains('users')) {
                    const userStore = db.createObjectStore('users', { keyPath: 'id' });
                    userStore.createIndex('username', 'username', { unique: true });
                }

                // Store: configurações (produtos, metas, regras)
                if (!db.objectStoreNames.contains('config')) {
                    const configStore = db.createObjectStore('config', { keyPath: 'key' });
                    configStore.createIndex('key', 'key', { unique: true });
                }
                
                // Store: registros diários de venda
                ...
                // Store: fiados (dívidas não quitadas)
                ...
                // Store: histórico (dados para possível treinamento de IA)
    ...
    // ========== CRUD GENÉRICO ==========
    // As quatro operações básicas reaproveitadas por todos os métodos específicos.
    ...
    // As configurações são salvas com chave composta: userId + chave original.
    // Isso permite isolar completamente os dados de cada usuário.
    ...
    // ========== CONFIGURAÇÃO PADRÃO ==========
    // Quando um novo usuário se cadastra, inicializamos suas configurações
    // com valores padrão (metas, produtos, regras de mix).

    async initDefaultConfigForUser(userId) {
        const metas = await this.getConfig(userId, 'metas');
        if (metas) return;  // Já inicializado, não sobrescreve
    ...
    // Baseado em um negócio real de doces (Doçuras de Artemis).
    // Perceba o uso de unidade_base_id e fator_conversao, que permitem
    // definir relações como "1 caixa = 4 brigadeiros avulsos".
    ...
    // ========== REGRAS DE MIX PADRÃO ==========
    // Essas regras permitem ao sistema de sugestão adaptar o mix
    // com base em condições como clima, dia da semana e temperatura.
    ...
    // Permitem gerar um backup completo dos dados em JSON e restaurá-lo.
...
// Cria instância global para ser usada em toda a aplicação
