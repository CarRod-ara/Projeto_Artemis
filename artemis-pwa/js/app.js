...
class ArtemisApp {
    ...
    async init() {
        try {
            ...
            this.metas = await db.getConfig('metas') || { survival: 110, comfortable: 150, ideal: 260 };
            ...
            this.setupModalListeners(); // ← Corrigido: moved from global scope
    ...
    async renderRegistrar(container) {
        ...
        const sugestaoMix = await mixEngine.generateSuggestion({
            data: hoje, diaSemana, clima: this.weatherData,
            temperatura: this.weatherData?.current?.temperature,
            produtos: this.produtos, regras: this.regrasMix
        });
                ...
                <form id="registroForm">
                    <div class="grid grid-cols-2 gap-3 mb-4">
                        <div>
                            ...
                            <input type="date" id="regData" value="${hoje}" required class="w-full px-3 py-2 border border-gray-300 rounded-lg">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-600 mb-1">Dia</label>
                            <input type="text" id="regDiaSemana" value="${diaSemana}" readonly class="w-full px-3 py-2 bg-gray-100 border border-gray-300 rounded-lg text-gray-500">
                        </div>
                    </div>
                    <div class="mb-4">
                        ...
                        <div class="grid grid-cols-2 gap-3">
                            <div>
                                ...
                                <input type="number" step="0.01" id="regPagos" required value="${registroExistente?.fluxo?.pagosDia ?? ''}" class="w-full px-3 py-2 border border-gray-300 rounded-lg">
                            </div>
                            <div>
                                <label class="block text-sm text-gray-600 mb-1">🟡 Fiados (R$)</label>
                                <input type="number" step="0.01" id="regFiados" value="${registroExistente?.fluxo?.fiadosHoje ?? '0'}" class="w-full px-3 py-2 border border-gray-300 rounded-lg">
                            </div>
                        </div>
                        <div class="mt-2">
                            <label class="block text-sm text-gray-600 mb-1">🔵 Recebidos (R$)</label>
                            <input type="number" step="0.01" id="regRecebidos" value="${registroExistente?.fluxo?.recebidosFiados ?? '0'}" class="w-full px-3 py-2 border border-gray-300 rounded-lg">
                        </div>
                    </div>
                    <div class="mb-4">
                        ...
                        <div class="grid grid-cols-2 gap-3">
                            <div>
                                ...
                                <input type="time" id="regInicio" required value="${registroExistente?.tempoOperacional?.inicio ?? ''}" class="w-full px-3 py-2 border border-gray-300 rounded-lg">
                            </div>
                            <div>
                                <label class="block text-sm text-gray-600 mb-1">Fim</label>
                                <input type="time" id="regFim" required value="${registroExistente?.tempoOperacional?.fim ?? ''}" class="w-full px-3 py-2 border border-gray-300 rounded-lg">
                            </div>
                        </div>
                    </div>
                    <div class="mb-4">
                    ...
                    <div class="mb-4">
                        ...
                        <div class="grid grid-cols-2 gap-3">
                            ...
                            <div>
                                ...
                                <input type="number" id="regClimaTemp" value="${this.weatherData?.current?.temperature ?? 25}" class="w-full px-3 py-2 border border-gray-300 rounded-lg">
                            </div>
                        </div>
                    </div>
                    <div class="mb-4">
                        <label class="block text-sm font-medium text-gray-600 mb-1">📝 Observações</label>
                        <textarea id="regObservacoes" rows="3" class="w-full px-3 py-2 border border-gray-300 rounded-lg" placeholder="O que aconteceu de diferente hoje?">${this.escapeHtml(registroExistente?.observacoes || '')}</textarea>
                    </div>
        ...
        document.getElementById('regData')?.addEventListener('change', (e) => {
            ...
            const diaSemanaNew = data.toLocaleDateString('pt-BR', { weekday: 'long' });
            document.getElementById('regDiaSemana').value = diaSemanaNew;
        });
    ...
    async salvarRegistro(event) {
        ...
        if (!data) { this.showToast('Data inválida', 'error'); return; }
        
        const itensVendidos = [];
        for (const produto of (this.produtos || [])) {
            const levado = this.safeNumber(document.querySelector(`[name="levado_${produto.id}"]`)?.value);
            const vendido = this.safeNumber(document.querySelector(`[name="vendido_${produto.id}"]`)?.value);
            if (levado > 0 || vendido > 0) {
                itensVendidos.push({
                    categoria: produto.categoria, codigo: produto.codigo,
                    nome: produto.nome, levado, vendido, precoUnitario: produto.preco
                });
            }
        }
        
        const eficiencia = this.calcularEficiencia(itensVendidos);
        const inicio = document.getElementById('regInicio')?.value;
        const fim = document.getElementById('regFim')?.value;
        
        const registro = {
            id: data, data,
            diaSemana: document.getElementById('regDiaSemana')?.value || '',
            fluxo: {
                pagosDia: this.safeNumber(document.getElementById('regPagos')?.value),
                fiadosHoje: this.safeNumber(document.getElementById('regFiados')?.value),
                recebidosFiados: this.safeNumber(document.getElementById('regRecebidos')?.value)
            },
            tempoOperacional: {
                inicio: inicio || '', fim: fim || '',
                totalMinutos: this.calcularMinutos(inicio, fim)
            },
            itensVendidos,
            clima: {
                real: {
                    condicao: document.getElementById('regClimaCondicao')?.value || 'nublado',
                    temperatura: this.safeNumber(document.getElementById('regClimaTemp')?.value, 25)
                },
                previsao: {
    ...
    async doImport() {
        ...
        try {
            ...
            if (data.registros) for (const r of data.registros) await db.saveRegistro(r);
            if (data.fiados) for (const f of data.fiados) await db.saveFiado(f);
            if (data.config) for (const [k, v] of Object.entries(data.config)) await db.setConfig(k, v);
            this.closeModal();
